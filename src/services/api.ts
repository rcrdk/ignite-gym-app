...
API.registerInterceptTokenManager = (signOut) => {
  const interceptTokenManager = API.interceptors.response.use(
    ...
    async (requestError) => {
      ...
      const isTokenError = requestError.response?.data?.message === 'token.expired' || requestError.response?.data?.message === 'token.invalid'
      ...
      if (requestError.response && requestError.response.data) {
        const errorMessage = new AppError(requestError.response.data.message, requestError.response.data.statusCode)
